...
function Home() {
  ...
  useEffect(() => {
    ...
    fetch("/Bug.json")
    .then((res) => {
      console.log("Fetching bug.json, status:", res.status, "URL:", res.url); // Line 47
      if (!res.ok) {
        throw new Error(`Failed to fetch bug.json: ${res.status}`); // Line 50
      }
      return res.json();
    })
    .then((data) => setBugs(data))
    .catch((err) => {
      console.error("Error loading bugs:", err); // Line 57
